...
public class ItemRepositoryCustomImpl implements ItemRepositoryCustom{
    ...
    private JPAQueryFactory queryFactory; // 동적쿼리 사용하기 위해 JPAQueryFactory변수 선언
    //생성자
    public ItemRepositoryCustomImpl(EntityManager em){
        this.queryFactory = new JPAQueryFactory(em); //JPAqueryFactory 실질적인 객체가 만들어 진다.
    }
    ...
    private BooleanExpression regDtsAfter(String searchDateType){//all, 1d 1w 1m 6m
        ...
        //dateTime은 시간에 맞게 세팅후 시간에 맞는 등록된 상품이 조회하도록 조건값 반환
    }
    ...
    public Page<MainItemDto> getMainItemPage(ItemSearchDto itemSearchDto, Pageable pageable){
        ...
        //select i.id,id.itemNm,i.itemDetail,im.itemImg, i.price form item i, itemimg im join i.id = im.itemid
        //where im.repImgYn = "Y" and i.itemNm like %searchQuery% order by i .id desc
        //QMianItemDto @QueryProcection을 허용하면 DTO로 바로 조회 가능
        QueryResults<MainItemDto> results = queryFactory.select(new QMainItemDto(item.id, item.itemNm,
                ...
                //join 내부조인 .repImpYn.eq("Y") 대표이미지만 가져온다.
