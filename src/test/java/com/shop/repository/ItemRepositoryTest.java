...
@SpringBootTest //스프링 부타 테스트
//테스트 관련 프로퍼티 위치야 -> application-test.properties
...
class ItemRepositoryTest {
    ...
    public void createItemTest(){
        ...
        // save -> insert 실행 결과가 insert 된 Item 객체가 나옴
    ...
    public void createItemList(){
        for(int i = 1; i<=10; i++){
            ...
            item.setStockNumber(100);
            item.setRegTime(LocalDateTime.now());
            itemRepository.save(item); //실제로 DB에 저장 10번 i<=10
        }
    }
    @Test
    ...
    public void querydslTest(){
        ...
        //JPAQuery를 사용하기 위해서는 QXXX가 필요 -> QueryDSL을 의존성 추가 -> 컴파일 -> 사용가능 설정
        JPAQuery<Item> query = queryFactory.selectFrom(qItem) // select * from item
                ...
                .where(qItem.itemDetail.like("%"+"테스트 상품 상세 설명"+"%")) // item_detail like 테스트 상품 상서 설명
    ...
    public void createItemList2(){
        for(int i = 1; i<=5; i++){
            ...
            item.setStockNumber(100);
            item.setRegTime(LocalDateTime.now());
            itemRepository.save(item); //실제로 DB에 저장 1~5번 i<=5
        }
        for(int i = 6; i<=10; i++){
            Item item = new Item();
            item.setItemNm("테스트 상품"+ i);
            ...
            item.setStockNumber(100);
            item.setRegTime(LocalDateTime.now());
            itemRepository.save(item); //실제로 DB에 저장 6~10번 i<=10
        }
    }
    @Test
    ...
    public void queryDslTest2(){
        ...
        BooleanBuilder booleanBuilder = new BooleanBuilder(); //SQL 쿼리문 조합하는 애
        ...
        //and => qItem.price 가 10003 초과인 값만
        booleanBuilder.and(qItem.price.gt(price)); //gt = 초과
        //StringUtils -> String SELL == ItemSellStatus.SELL
        if(StringUtils.equals(itemSellStat,ItemSellStatus.SELL)){
            //and qItem.itemSellStatus == itemSellStatus.SELL
            booleanBuilder.and(qItem.itemSellStatus.eq(ItemSellStatus.SELL));
        }
        //Pageable 객체를 생성하는 시작 인덱스 0이고 사이즈 5로 생성 됩니다.
        Pageable pageable = PageRequest.of(0,5);
        //itemRepository.findAll 모두 검색 1 매개변수 -> 쿼리 조건이 들어가고 , 2번째 매개변수에는 -> 페이지 세팅
        //결과 Page<Item> 결과를 받습니다.
        //결과 값 개수가 있고 결과 값도 같이 가지고 있습니다.
        Page<Item> itemPagingResult = itemRepository.findAll(booleanBuilder, pageable);
        //getTotalElements() -> 결과 값 개수 나옵니다.
